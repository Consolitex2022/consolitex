            ...
            <Box sx={styles.mobileContainer}>
                ...
                <Box sx={styles.mobileTextContainer}>
                    ...
                    <Box sx={styles.mobileButtonContainer}>
                        <Button component="a" target="_blank" href={`https://api.whatsapp.com/send?text=${utf8_encode(ucfirst(data.descripcion_web.toLowerCase()))}...%0AConsolitex%20%7C%20Bienes%20Raices%20https://consolitex.org/inmueble?id=${data.ficha_id}`} color="success" sx={styles.mobileButton}>
                            <WhatsApp />
                        </Button>
                        ...
                        <Button component="a" target="_blank" href={`https://twitter.com/intent/tweet?text=${utf8_encode(ucfirst(data.descripcion_web.toLowerCase()))}...%0AConsolitex%20%7C%20Bienes%20Raices%20https://consolitex.org/inmueble?id=${data.ficha_id}&hashtags=Consolitex`} color="info" sx={styles.mobileButton}>
                            <Twitter />
                        </Button>
                        <Button component="a" target="_blank" href={`mailto:?&subject=%20CONSOLITEX%20%7C%20INMUEBLE&body=${utf8_encode(ucfirst(data.descripcion_web.toLowerCase()))}...%0AConsolitex%20%7C%20Bienes%20Raices%0Ahttps://consolitex.org/inmueble?id=${data.ficha_id}`} color="primary" sx={styles.mobileButton}>
                            <Mail sx={{ color: "black" }} />
                        </Button>
                    </Box>
                </Box>
        ...
        <Box sx={styles.pcContainer}>
            <Box sx={styles.pcSubContainer}>
                <Box sx={styles.pcAvatar}>
                    <Avatar src={`https://consolitex.org/SISGACI/assets/img/${userData.foto}`} sx={{ width: { xs: 50, md: 100, lg: 150, }, height: { xs: 50, md: 100, lg: 150, } }} />
                </Box>
                <Box sx={styles.pcDataContainer}>
                    <Typography variant="overline" fontWeight="bold" color="common.white" sx={{ fontFamily: "Plus Jakarta Sans" }} fontSize={{ md: 12, lg: 14 }}>Recomendado por</Typography>
                    <Typography variant="subtitle2" color="common.white" sx={styles.pcDataText}>{`${userData.nombre_y_apellido}`}</Typography>
                    <Box sx={{ display: 'flex', alingItems: "center" }}>
                        <LocalPhoneIcon sx={{ color: "#FFF", width: 18, height: 18 }} />
...
const styles = {
    ...
    mobileTextContainer: { width: "100%", height: "50%", display: "flex", justifyContent: "center", flexFlow: "column wrap" },
    ...
    mobileDataContainer: { width: "65%", height: "100%", display: "flex", flexFlow: "column wrap", justifyContent: "center", pl: 1 },
    pcContainer: {
        display: {
            xs: "none",
            sm: "block"
    ...
    pcAvatar: { width: { xs: "40%", md: "35%", lg: "45%", xl: "50%" }, display: "flex", justifyContent: "end", },
    pcDataContainer: { p: 2, width: { xs: "60%", md: "65%", lg: "55%", xl: "50%" }, },
    pcDataText: { fontFamily: "Plus Jakarta Sans", fontSize: { md: 12, lg: 12, xl: 16 } }
}